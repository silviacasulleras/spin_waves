...
def plot_dispersion_rel():
    ...
    fig = plt.figure(figsize=(3.33*cm, 2*cm))
    ...
    plt.ylim(6.1,7.7)
    plt.xlim(0,25)
    plt.ylabel("$\omega/(2\pi) $ [GHz]",labelpad=2)
    ...
    cbar.set_label(' log($|m_z(k,0,0,\omega)/M_s|^2$)', rotation=-90, labelpad=12,color='black')
...
def plot_transfer_function(xpoint):
    ...
    ax1.vlines(omega_c/(2*np.pi*1e9), -0.1, 0.75, linestyle="dashed",color=c["green"],linewidth=0.8)
...
def plot_backwards_evolved_chirped_pulse(waveguide,chirped_pulse_initial):
    # Backwards evolved pulse
    
    Amplitude_pulse = np.load("simulations/chirped_pulse/Amplitude_pulse.npy")
    Xsize = waveguide["Xsize"]
    Nx    = waveguide["Nx"]
    xlist = np.linspace(-Xsize/2,Xsize/2,Nx)
    x_f   = chirped_pulse_initial["df"]-5*1e-6
    sigma_f = chirped_pulse_initial["sigmaf"]
    
    initial_pulse = np.load("simulations/chirped_pulse/initial_pulse.npy")
    target_pulse  = np.load("simulations/chirped_pulse/target_pulse_analytical.npy")
    target_envelope = np.load("simulations/chirped_pulse/envelope_target_pulse_analytical.npy")
...
def plot_evolved_chirped(waveguide,chirped_pulse_initial):
     
    Xsize = waveguide["Xsize"]

    #xlist = np.linspace(-Xsize/2,Xsize/2,Nx)
    
    m_pulse = np.load("simulations/chirped_pulse/m(x,t)_alone.npy")
    Treal   = np.load("simulations/chirped_pulse/T.npy")
    t_max_fidelity = np.load("simulations/chirped_pulse/time_max_fidelity.npy")
   
    cm = 1/2.54
    SMALL_SIZE = 8
 
    #Plot the dispersion relation and its quadratic approximation
    fig =  plt.figure(figsize=(7.25*cm, 3.75*cm))
    ...
    plt.xlim(-25,25)
    plt.ylim(0,125)
    plt.title("Target pulse evolution",size=8)
    ...
    plt.hlines((t_max_fidelity)*1e9,-30,30,color=c["blue"], linestyle="dashed",linewidth=0.8)    
   # cbaxes = fig.add_axes([0.87, 0.1, 0.04, 0.6])
    #cbar = plt.colorbar(aspect=20,cax=cbaxes)
    cbar = plt.colorbar(aspect=18)
   # cbar.set_label('$|\\tilde{m}_z(x,0,0,t)/M_s|^2$', rotation=90, labelpad=-28, color='white')
    cbar.set_label('$|\\tilde{m}_z(x,0,0,t)/M_s|^2$', rotation=-90, labelpad=12, color='black')
    ...
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
    plt.savefig("plots/plots_article/evolution_chirped.pdf", bbox_inches = 'tight',transparent=True,dpi=300)
    plt.savefig("plots/plots_article/evolution_chirped.svg")
    
    plt.show()


def plot_evolved_chirped_2(waveguide, chirped_pulse_initial):
    Xsize = waveguide["Xsize"]
    ...
    # Plot the dispersion relation and its quadratic approximation
    fig = plt.figure(figsize=(7.25 * cm, 4.75 * cm))
    ...
    extent = [- 1e6 * Xsize / 2, 1e6 * Xsize / 2, 0, 1e9 * Treal]
    plt.imshow(np.transpose(np.abs(m_pulse) ** 2), extent=extent, aspect='auto', origin='lower', cmap="viridis", vmin=0,
               vmax=5 * 1e-7)
    plt.ylabel("t [ns]", labelpad=0)
    plt.xlabel("x [$\mu$m]", labelpad=0)
    plt.xlim(-20, 15)
    ...
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
    plt.savefig("plots/plots_article/evolution_chirped_no_lines.pdf", bbox_inches='tight', transparent=True, dpi=300)
    plt.savefig("plots/plots_article/evolution_chirped_no_lines.svg")

    plt.show()


def plot_dispersion_rel_and_chirped_pulse():
    ...
    Ck_list         = np.load("simulations/chirped_pulse/Ck_list.npy") 
   ...
   # plt.plot(klist/1e6,1*omega_c/(2*np.pi*1e9)+10*np.abs(Ck_list)*1e8,color=c["blue"], label="pol. fit",linewidth=1.2)
    ck = 1*omega_c/(2*np.pi*1e9)+15*np.abs(Ck_list)*1e8
    plt.fill_between(klist/1e6, omega_c/(2*np.pi*1e9), ck, alpha=0.5,color=c["blue"],linewidth=0)  
    plt.ylim(omega_c/(2*np.pi*1e9),7.75)
    plt.xlim(0,25)
    plt.text(2,7.5,"target pulse",color=c["blue"])
    plt.tick_params(direction='in')
    plt.ylabel("$\omega/(2\pi) $ [GHz]",labelpad=0)
    plt.xlabel("$k\,\, [\mu m^{-1}] $",labelpad=0)
    plt.subplots_adjust(left=0.0, right=1, top=1, bottom=0)
    
    plt.savefig("plots/plots_article/dispersion_relation_+chirped_pulse.pdf", bbox_inches = 'tight',transparent=True,dpi=300)
    plt.savefig("plots/plots_article/dispersion_relation_+chirped_pulse.svg")
    plt.show()
 

    
def plot_driving_chirped_freq(waveguide,dispersion):   
    ...
    driving_new_units = A_a/(Zsize*By)*driving["V_spectrum"]
    ...
    fig, ax1 = plt.subplots()
    fig.set_figheight(2*cm)
    fig.set_figwidth(3*cm)
    
    color = c["blue"]
    ax1.set_xlabel("$\omega/(2\pi)$ [GHz]",size=SMALL_SIZE,labelpad=0)
    ax1.set_ylabel("$|V(\omega)| $ [ps]",color=color,size=SMALL_SIZE,labelpad=-0.2)
    ...
    plt.savefig("plots/plots_article/driving_spectrum_chirped.pdf", bbox_inches = 'tight',transparent=True)
    plt.savefig("plots/plots_article/driving_spectrum_chirped.svg")    
    plt.show()
...
def plot_generated_chirped_pulse(waveguide,chirped_pulse_initial):
    ...
    dt      = np.load("simulations/pulse_generation_chirped/dt.npy")  
    t0      = chirped_pulse_initial["t0"]
    Treal   = np.load("simulations/chirped_pulse/T.npy")
    t_max_fidelity = np.load("simulations/chirped_pulse/time_max_fidelity.npy")
   ...
   # cbaxes = fig.add_axes([0.87, 0.1, 0.04, 0.6])
    ...
    cbar = plt.colorbar(aspect=18)
   # cbar.set_label('$|\\tilde{m}_z(x,0,0,t)/M_s|^2$', rotation=90, labelpad=-28, color='white')
    cbar.set_label('$|m_z(x,0,0,t)/M_s|^2$', rotation=-90, labelpad=12, color='black')
    cbar.outline.set_edgecolor('black')
    plt.tick_params(direction='in',color="white")
    #cbar.outline.set_linewidth(1)
    #cbaxes.tick_params(axis='both', colors='white')
    ...
    plt.subplots_adjust(left=0, right=1, top=1, bottom=0)
    plt.savefig("plots/plots_article/generated_chirped.pdf", bbox_inches = 'tight', transparent="True",dpi=300)
    plt.savefig("plots/plots_article/generated_chirped.svg")
...
def plot_comparison_chirped(waveguide):
    # Show m_z(x,t) at different times compared to the target state    

    target_pulse    = np.load("simulations/chirped_pulse/target_pulse_normalized.npy")
    target_envelope    = np.load("simulations/chirped_pulse/envelope_target_pulse_normalized.npy")
    generated_pulse = np.load("simulations/pulse_generation_chirped/m(x,time_max_fidelity)_normalized.npy")
    xlist_half      = np.load("simulations/chirped_pulse/xlist_half.npy")
    fidelity        = np.load("simulations/pulse_generation_chirped/fidelity_protocol_sigma=30nm.npy")
    Zsize = waveguide["Zsize"]
    ...
    plt.plot(1e6*xlist_half,generated_pulse*1e-3,label="generated",color=c["green"])        
    plt.plot(1e6*xlist_half,target_pulse*1e-3,label="target",color=c["blue"],linestyle="dashed")
    env = target_envelope*1e-3
    #plt.plot(1e6*xlist_half,  env, alpha=0.25,color=c["blue"])      
    plt.fill_between(1e6*xlist_half, - env, env, alpha=0.2,color=c["blue"], linewidth=0.0)  
    plt.ylim(-2.7,2.7)
    plt.xlim(5,8.5)
    plt.tick_params(direction='in')
    plt.text(8.4,2,f"""Fidelity={fidelity:.3f}""",ha='right', fontsize=7)
    plt.legend(loc="upper left",frameon=False, labelspacing=0.1, borderpad=-0.2)
    plt.ylabel("$m_z(x,0,0,t_f)/N$ \n [$\sqrt{\mu  \mathrm{m} }^{-1}$]",labelpad=0)
    plt.xlabel("x [$\mu$m]",labelpad=0)
    
    plt.subplots_adjust(left=0.0, right=1, top=1, bottom=0.0)
    
    plt.savefig("plots/plots_article/comparison_chirped.pdf", bbox_inches = 'tight',transparent=True)
    plt.savefig("plots/plots_article/comparison_chirped.svg")    
    plt.show()
    '''
    cm = 1/2.54
    SMALL_SIZE = 8
    ...
    plt.xlim(5,8.5)
    plt.tick_params(direction='in')
    plt.text(8.4,0.34,f"""Fidelity={fidelity:.3f}""",ha='right', fontsize=7)
    plt.legend(loc="upper left",frameon=False, labelspacing=0.1, borderpad=-0.2)
    plt.ylabel("$m_z(x,0,0,t_f)\sqrt{L_z}/N$",labelpad=0)
    plt.xlabel("x [$\mu$m]",labelpad=0)
