...
def target_square_pulse_spectrum(waveguide,  ground_state, dispersion, evolved_pulse, transfer_func):
    ...
    #Import the magnetization of the square pulse in temporal domain 
    ...
    plt.savefig("plots/square_pulse/m_vs_t_x=0.pdf",bbox_inches='tight')
    plt.show()
    ...
    plt.savefig("plots/square_pulse/spectrum_pulse.pdf")
    plt.show()
...
def determine_driving_squared(waveguide,dispersion,transfer_func,pulse_spectrum):
    ...
    plt.ylabel("$V(t)$")
    plt.xlabel("$t$ [ns]")
    plt.savefig("plots/pulse_generation_square/V_time.pdf")    
    plt.show()


    #Export the values of the driving in time and spectral domain

    np.save("simulations/pulse_generation_square/V(omega)_freqs", V_spectrum["omega"])
    np.save("simulations/pulse_generation_square/V(omega)_values", V_spectrum["values"])
    np.save("simulations/pulse_generation_square/domega", V_spectrum["domega"])


    np.save("simulations/pulse_generation_square/V(t)_time", V_time["t"])
    np.save("simulations/pulse_generation_square/V(t)_values", V_time["values"])
    np.save("simulations/pulse_generation_square/T_driving",  V_time["T_driving"])    
    np.save("simulations/pulse_generation_square/T_driving_initial",  V_time["T_driving_initial"])
    np.save("simulations/pulse_generation_square/dt_driving", V_time["dt_driving"])
...
def import_driving_squared(waveguide):   
    ...
    plt.xlabel("$\omega/(2\pi)$ [GHz]")
    plt.xlim(6,8)
    plt.savefig("plots/pulse_generation_square/V(omega).pdf")
    plt.show()


    #Delta_t_driving = np.load(f"simulations/pulse_generation_square/Delta_t_driving.npy")  
    V_t = driving["V_time"] 
    Delta_t_driving = 53*1e-9
    np.save("simulations/pulse_generation_square/Delta_t_driving_0.995",Delta_t_driving)  
    ...
    plt.savefig("plots/pulse_generation_square/V_time_all.pdf")
    plt.show()
    ...
    np.save(f"""simulations/pulse_generation_square/B_max_sigma={int(sigma_a*1e9)}nm""",B_max)  
    np.save(f"""simulations/pulse_generation_square/U_total_sigma={int(sigma_a*1e9)}nm""",U_total) 
    np.save(f"""simulations/pulse_generation_square/U_0={int(sigma_a*1e9)}nm""",U_0)  
...
def simulate_generation_squared_pulse(waveguide, driving_N_square):
    ...
    V_t         = np.real(driving_N_square["V_time"])
...
"""
    ...
    create_mumax_script(script,"simulations/pulse_generation_square/","pulse_generation_square")
    print("Mumax3 script pulse_generation_square.txt created")



def import_generated_squared_pulse(waveguide, ground_state, driving, xpoint, evolved_pulse, target_pulse_spectrum):
    ...
    mzfields = read_npy_files("m","simulations/pulse_generation_square/pulse_generation_square.out")
    ...
    #np.save("simulations/pulse_generation_square/m(x,t)", mz0)
    np.save("simulations/pulse_generation_square/m(x,t)_alone", mz0 - mground_xt)
    np.save("simulations/pulse_generation_square/dt", dt)    
    np.save("simulations/pulse_generation_square/mzground_all", mground_xt)
...
def show_generated_squared_pulse(waveguide, ground_state, driving, xpoint, evolved_pulse, target_pulse_spectrum):
    ...
    sigma_a    = waveguide["sigma_antenna"]
    time_fidelity_max_e   = np.load("simulations/square_pulse/time_max_fidelity.npy") 
    ...
    i_time_fidelity_max_e = find_nearest(tlist_real, time_fidelity_max_e)["index"]  
    fidelity_evolution  = np.load("simulations/square_pulse/fidelity_evolution.npy") 
    
    target_analytical   = np.load("simulations/square_pulse/target_pulse_analytical.npy") #Analytical target pulse
    envelope_analytical = np.load("simulations/square_pulse/envelope_target_pulse_analytical.npy") #Analytical target pulse
    ...
    plt.imshow(np.transpose(np.abs(m_pulse_generated)**2), extent=extent, aspect='auto', origin='lower', cmap="viridis",vmin=0,vmax=0.0000015)
    plt.ylabel("t [ns]")
    plt.xlabel("x [$\mu$m]")
    plt.colorbar()
    plt.title("$|m_z(x,y=0,z=0,t)|^2$" )
    plt.vlines(0,0,150,color="white", linestyle="dashed",linewidth=0.5)  
    plt.hlines(time_fidelity_max_e*1e9,-30,30,color="white", linestyle="dashed",linewidth=0.5)  
    plt.xlim(-20,20)
    plt.ylim(0,120)
    #plt.hlines(20,-40,40,color="white", linestyle="dashed")
    plt.savefig("plots/pulse_generation_square/m_abs_created+lines.pdf",bbox_inches='tight',dpi=300)
    plt.show()
    ...
    for i_t in range(find_nearest(tlist_real, time_fidelity_max_e-25*dt)["index"],find_nearest(tlist_real,time_fidelity_max_e+25*dt)["index"]+1,1):
        t = i_t*dt
    ...
    plt.xlabel("t [ns]")
    plt.ylim(0,1)       
    plt.xlim((time_fidelity_max_e-15*dt)*1e9,(time_fidelity_max_e+15*dt)*1e9)
    plt.vlines(time_fidelity_max_e*1e9,0,5,color=c["green"],linestyle='dashed')
    #plt.legend()    
    plt.savefig("plots/pulse_generation_square/fidelity_vs_time.pdf")
    plt.show()
    ...
    plt.xlim(8,12)
    plt.legend()    
    plt.savefig("plots/pulse_generation_square/pulses_at_maximum_fidelity.pdf",bbox_inches='tight')
    plt.show() 
    ...
    #plt.fill_between(1e6*xlist_half, 0,- env, alpha=0.4,color=c["green"])
    plt.ylabel("$m_z(x,t)/N$ [$\mu$m]$^{-1/2}$")
    plt.xlabel("x [$\mu$m]")
    plt.xlim(8,12)
    plt.legend()    
    plt.savefig("plots/pulse_generation_square/pulses_at_maximum_fidelity_N.pdf",bbox_inches='tight')
    plt.show() 
    
    np.save(f"""simulations/pulse_generation_square/fidelity_protocol_sigma={int(sigma_a*1e9)}nm""",fidelity_max)   
    np.save("simulations/pulse_generation_square/m(x,time_max_fidelity)", mpulse_max_fidelity)    
    
    np.save("simulations/pulse_generation_square/m(x,time_max_fidelity)_normalized", mpulse_max_fidelity/N_normalize_generated)   
    np.save("simulations/square_pulse/target_pulse_normalized",target_analytical_half_space/N_normalize_target)   
    np.save("simulations/square_pulse/envelope_target_pulse_normalized",envelope_analytical_half_space/N_normalize_target)   
    np.save("simulations/square_pulse/xlist_half",xlist_half)   
    ...
    pulse_evolved_at_xf = np.load("simulations/square_pulse/pulse_vs_time_at_xf.npy")
    pulse_evolved       = np.load("simulations/square_pulse/m(x,t)_alone.npy")
    ...
    plt.plot(1e9*tlist_real,np.abs(pulse_evolved_at_xf)**2,label="evolved",color=c["green"],alpha=0.5)   
    ...
    plt.savefig("plots/pulse_generation_square/pulse_xf_vs_time.pdf",bbox_inches='tight')
    plt.show() 
    ...
    #plt.xlim(65,75)
    #plt.hlines(np.sqrt(var_time_max_F)*1e9,0,30, color=c["green"],linestyle="dashed")
    plt.savefig("plots/pulse_generation_square/pulse_xf_vs_time_all.pdf",bbox_inches='tight')
    plt.show()
    ...
    plt.ylim(66,74)
    #plt.hlines(20,-40,40,color="white", linestyle="dashed")
    plt.savefig("plots/pulse_generation_square/m_abs_created+lines_detail.pdf",bbox_inches='tight',dpi=300)
    plt.show()
    ...
    plt.xlim(0,30)
    plt.hlines(np.sqrt(var_time_max_F)*1e9,0,30, color=c["green"],linestyle="dashed")
    plt.savefig("plots/pulse_generation_square/stardard_deviation_time(t).pdf",bbox_inches='tight')
    plt.show() 
    ...
    plt.savefig("plots/pulse_generation_square/stardard_deviation(t).pdf",bbox_inches='tight')
    plt.show()  
    
    #Variance at the time of maximum fidelity    

    
    np.save(f"""simulations/pulse_generation_square/variance_at_max_fidelity_sigma={int(sigma_a*1e9)}nm""",var_time_max_fidelity)  
    #np.save(f"""simulations/pulse_generation_square/min_variance_sigma={sigma_a}""",var_min)    
